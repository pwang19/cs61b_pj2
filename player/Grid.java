 ...
 * @throws InvalidMoveException 
  ...
  **/
  ...
  /** 
   * isValidMove tells if it's a valid move
   * @param side
   * @param x
   * @param y
   * @return boolean
  ...
  protected boolean isValidMove(int side, int x, int y) {
	  ...
	  if (board[h-1][g-1] == side && visited[h-1][g-1] == false)  {
		  if ((h-1) != x || (g-1) != y) {
			  stotal++;
			  a = h-1;
			  b = g-1;
			  visited[h-1][g-1] = true;
			  System.out.println("bad");
		  }
    	}	  
	  if (board[h-1][g] == side && visited[h-1][g] == false) {
		  if ((h-1) != x || (g) != y) {
			stotal++;
		  	a = h-1;
		  	b = g;
	  ...
	  if (board[h-1][k+1] == side && visited[h-1][k+1] == false) {
		  if ((h-1) != x || (k+1) != y) {
			  System.out.println("why");
			  stotal++;
			  a = h-1;
			  b = k+1;
			  visited[h-1][k+1] = true;
		  }
	  }
      if (board[h][g-1] == side && visited[h][g-1] == false) {
    	  if (h != x || (g-1) != y) {
    		  stotal++;
    		  a = h;
    		  b = g-1;
      ...
      if (board[h][k+1] == side && visited[h][k+1] == false) {
    	  if (h != x || (k+1) != y) {
    		  stotal++;
    		  a = h;
    		  b = k + 1;
      ...
      if (board[j+1][g-1] == side && visited[j+1][g-1] == false) {
    	  System.out.println("not");
    	  if ((j+1) != x || (g-1) != y) {
    	  	stotal++;
    	  	a = j+1;
    	  	b = g-1;
    	  	visited[j+1][g-1] = true;
    	  }
      }
      if (board[j+1][g] == side && visited[j+1][g] == false) {
    	  if ((j+1) != x || g != y) {
    		  stotal++;
    		  a = j+1;
    		  b = g;
      ...
      if (board[j+1][k+1] == side && visited[j+1][k+1] == false) {
    	  if ((j+1) != x || (k+1) != y) {
    		  stotal++;
    		  a = j+1;
    		  b = k+1;
      ...
      if (stotal == 1) {
    	  ...
    	  if (board[h-1][g-1] == side && visited[h-1][g-1] == false)  {
    		  if (((h-1) != a || (g-1) != b) && ((h-1) != x || (g-1) != y))  {
    			  stotal++;
    			  visited[h-1][g-1] = true;
    		  }
	      }
	  	  if (board[h-1][g] == side && visited[h-1][g] == false) {
	  		if (((h-1) != a || (g) != b) && ((h-1) != x || g != y)) {
	  		  stotal++;
	  		  visited[h-1][g] = true;
	  		}
	  	  }
	  	  if (board[h-1][k+1] == side && visited[h-1][k+1] == false) {
			  if (((h-1) != a || (k+1) != b) && ((h-1) != x || (k+1) != y)) {
				  stotal++;
				  visited[h-1][k+1] = true;
			  }
	  	  }
	      if (board[h][g-1] == side && visited[h][g-1] == false)  {
			  if ((h != a || (g-1) != b) && (h != x || (g-1) != y)) {
				  stotal++;
				  visited[h][g-1] = true;
			  }
	      }
	      if (board[h][k+1] == side && visited[h][k+1] == false) {
	    	  if ((h != a || (k+1) != b) && (h != x || (k+1) != y)) {
	    		  stotal++;
	    		  visited[h][k+1] = true;
	    	  }
	      }
	      if (board[j+1][g-1] == side && visited[j+1][g-1] == false) {
	    	  if (((j+1) != a || (g-1) != b) && ((j+1) != x || (g-1) != y)) {
	    		  stotal++;
	    		  visited[j+1][g-1] = true;
	    	  }
	      }
	      if (board[j+1][g] == side && visited[j+1][g] == false) {
	    	  if (((j+1) != a || g != b) && ((j+1) != x || g != y)) {
	    		  stotal++;
	    		  visited[j+1][g] = true;
	    	  }
	      }
	      if (board[j+1][k+1] == side && visited[j+1][k+1] == false) {
	    	  if (((j+1) != a || (k+1) != b) && ((j+1) != x || (k+1) != y)) {
	    		  stotal++;
	    		  visited[j+1][k+1] = true;
	    	  }
	      }
  ...
  private int[][] listConnections(int x, int y){
		...
		//checking for connections and blocks
			...
			for (int j = 0; j < sideCoordinates.length; j++){
				...
				if (opposingCoordinates[j][0] == x && opposingCoordinates[j][1] > y && sideCoordinates[j][1] < h){
			...
			while(a1 >= 0 && c1 < 8 && b1 >=0 && d1 < 8){
				...
				if (opposingCoordinates[j][0] == a1 && opposingCoordinates[j][1] == b1 && a1 > o){
					o = a1; p = b1;
				}
				...
				if (opposingCoordinates[j][0] == c1 && opposingCoordinates[j][1] == b1 && b1 > xx){
					r = c1;	xx = b1;
				}
				...
				if (opposingCoordinates[j][0] == c1 && opposingCoordinates[j][1] == d1 && d1 < z){
					yy = c1; z = d1;
				}
				}
			a1 = x; b1 = y; c1 = x; d1 = y;
  ...
  **/
  public static void main(String[] args) throws InvalidNodeException  {
    ...
    System.out.println(g1.board[2][2] + "" + array[2][2]);
      ...
      System.out.println(g1.hasValidNetwork(0));
